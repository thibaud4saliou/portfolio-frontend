...
export default function AdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent, type: string) => {
    ...
    const url = editingId ? `http://localhost:5001/api/${type}/${editingId}` : `http://localhost:5001/api/${type}`;

    try {
      const res = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(bodyData) });
      if (res.ok) {
        setStatus(editingId ? "✅ Modification sauvegardée !" : "✅ Ajouté avec succès !");
        cancelEdit();
        fetchItems(type); 
    ...
    <div className="max-w-5xl mx-auto pt-12 pb-24 px-6">
      ...
      <div className="flex flex-wrap gap-4 mb-8 justify-center border-b border-gray-900 pb-4">
        {['projects', 'photos', 'reviews', 'equipment'].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`text-xs uppercase tracking-widest px-4 py-2 ${activeTab === tab ? 'bg-white text-black font-bold' : 'text-gray-500 hover:text-white'}`}>
